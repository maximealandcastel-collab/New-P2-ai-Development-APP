...
import { MediaCleanup } from './mediaCleanup.service';
 ...
 * Mux deletion is queued for the worker; other storage remains an operator task.
    ...
    await session.withTransaction(async () => {
      ...
      if (trainer) {
        ...
        for (const item of content) {
          for (const [field, kind] of [['muxAssetId', 'mux_asset'], ['muxUploadId', 'mux_upload']]) {
            if (typeof item[field] !== 'string' || !item[field]) continue;
            hasMuxCleanup = true;
            await MediaCleanup.updateOne({ kind, providerId: item[field] }, { $setOnInsert: {
              userId: objectId, contentId: String(item._id), kind, providerId: item[field],
              state: 'pending', attempts: 0, nextAttemptAt: new Date(), leaseUntil: new Date(0),
            } }, { upsert: true, session });
          }
        }
      ...
      await AccountDeletionCleanup.updateOne({userId:objectId},{$set:{assets:[...new Set(assets)],deletedAt:new Date(),status:assets.length || hasMuxCleanup?'pending_external_cleanup':'complete'}},{upsert:true,session});
