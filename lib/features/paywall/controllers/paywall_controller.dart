import 'dart:developer';
    ...
    import 'package:p2p_fitness/features/nav_bar/presentation/screens/nav_bar.dart';
    ...
    Future<void> applyPromoCode() async {
      ...
      try {
        ...
        if (!response.isSuccess) {
          promoError.value = response.errorMessage.isNotEmpty == true
        ...
        log("Promo code validated: ${code}  type=${codeType}");

        // 2. Website codes (purchased on p2pfitechai.com) — redeem directly,
        //    no IAP/Stripe needed since the customer already paid on the website.
        if (codeType == "website") {
          final redeemResp = await _networkCaller.postRequest(
            AppUrls.promoRedeem,
            body: {"code": code},
          );
          if (redeemResp.isSuccess) {
            ...
            promoError.value = redeemResp.errorMessage.isNotEmpty == true
                ? redeemResp.errorMessage
                : "Could not redeem code. It may already be used.";
            appliedPromoCode.value = "";
          }
          return;
        }

        // 3. Affiliate / other codes — apply discount to IAP/Stripe price as before
